#!/usr/bin/env python3
"""
Shared-Metabolite Edge Sparsification Sensitivity Analysis
============================================================
Evaluates MetaGNN performance across varying levels of R-R edge
density in the shared_metabolite relation, answering:

  "How does top-k neighbour sparsification of the shared-metabolite
   reaction-reaction edges affect reconstruction quality?"

Experimental design:
  - k in {0, 5, 10, 20, 50, 100, ALL}
    k=0  means shared_metabolite edges are REMOVED entirely
         (bipartite-only: substrate_of + produces)
    ALL  means no sparsification (full 7.5M edges; will OOM on
         <=32 GB GPUs, included for completeness on larger hardware)
  - 3-fold CV per k (faster than 5-fold; enough for sensitivity)
  - Fixed architecture: Config B (256/3/8) from the paper
  - Fixed seed per fold for fair comparison across k values
  - Reports: F1, AUROC, AUPRC, peak GPU memory, wall-clock time

Output:
  results_k_sensitivity/
    k_sensitivity_results.csv      - per-fold metrics for all k
    k_sensitivity_summary.csv      - mean +/- std per k
    k_sensitivity_plot.png         - bar chart with error bars

Usage:
  python exp_shared_edge_sensitivity.py --data_root /path/to/processed_690

  # Quick test with fewer epochs:
  python exp_shared_edge_sensitivity.py --data_root /path/to/processed_690 --n_epochs 30

  # Specify which k values to test (comma-separated):
  python exp_shared_edge_sensitivity.py --data_root /path/to/processed_690 --k_values 0,5,10,20,50

  # Skip the 'all' setting to avoid OOM:
  python exp_shared_edge_sensitivity.py --data_root /path/to/processed_690 --skip_all
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('k_sensitivity.log', mode='w', encoding='utf-8'),
    ]
)
...
# ---------------------------------------------------------------------------
# Model (same as run_full_cohort_experiments.py)
# ---------------------------------------------------------------------------
...
def sparsify_shared_edges(shared_ei: torch.Tensor, k: int) -> torch.Tensor:
    """Keep top-k random neighbours per source node. Fully vectorised."""
    n_orig = shared_ei.shape[1]
    n_nodes = shared_ei.max().item() + 1
    if k >= n_orig // max(n_nodes, 1):
        return shared_ei  # already sparse enough
    src = shared_ei[0]
...
# ---------------------------------------------------------------------------
# Dataset (same as run_full_cohort_experiments.py, but k-parameterised)
# ---------------------------------------------------------------------------
class KSensitivityDataset(torch.utils.data.Dataset):
    """Load MethodsX-format dataset with configurable shared_metabolite k."""

    def __init__(self, data_root, patient_ids, k_shared='all'):
        """
        Args:
            k_shared: int (top-k per node), 0 (remove R-R edges), or 'all'
        """
        ...
        # Shared metabolite edges: controlled by k_shared
...
def run_single_fold(data_root, k_shared, train_ids, val_ids, test_ids,
    ...
    """Train and evaluate one fold for a given k value."""
    ...
    # Build datasets with specified k
...
# ---------------------------------------------------------------------------
# Main: sweep over k values with 3-fold CV
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(
        description='Shared-metabolite edge sparsification sensitivity analysis')
    parser.add_argument('--data_root', type=str, required=True)
    parser.add_argument('--output_dir', type=str, default='./results_k_sensitivity')
    parser.add_argument('--k_values', type=str, default='0,5,10,20,50,100',
                        help='Comma-separated k values to test (0=no R-R edges)')
    parser.add_argument('--skip_all', action='store_true',
                        help='Skip k=all (avoids OOM on <=32GB GPUs)')
    ...
    # Check original edge count
    shared_path = os.path.join(args.data_root, 'edge_indices', 'shared_metabolite.pt')
    if os.path.exists(shared_path):
        orig_ei = torch.load(shared_path, weights_only=True)
        logger.info(f"Original shared_metabolite edges: {orig_ei.shape[1]:,}")
        del orig_ei

    # CV splits (same across all k for fair comparison)
    ...
    for k in k_values:
        ...
        for fold_idx, (train_val_idx, test_idx) in enumerate(folds):
            ...
            train_val_strat = [strat[i] for i in train_val_idx]
            test_ids = [patient_ids[i] for i in test_idx]

            # Use a secondary split for val
            ...
            val_ids = [train_val_ids[i] for i in range(len(train_val_ids)) if val_mask[i]]
            ...
            try:
                ...
                logger.warning(f"    OOM at k={k}, fold {fold_idx+1} -- skipping remaining folds for k={k}")
                all_results.append({
                    'k_shared': str(k), 'fold': fold_idx + 1,
    ...
    summary_rows = []
    for k in k_values:
        subset = df[df['k_shared'] == str(k)]
        valid = subset.dropna(subset=['F1_mean'])
        if len(valid) == 0:
            summary_rows.append({'k': str(k), 'n_folds': 0, 'note': 'OOM'})
            continue
        row = {
            'k': str(k),
            'n_folds': len(valid),
            ...
            'AUPRC_mean': valid['AUPRC_mean'].mean(),
            'AUPRC_std': valid['AUPRC_mean'].std(),
            'avg_train_time_s': valid['train_time_s'].mean(),
            'avg_peak_gpu_mb': valid['peak_gpu_mb'].mean(),
        }
    ...
    try:
        ...
        if len(valid_summary) >= 2:
            ...
            plt.suptitle('Shared-Metabolite Edge Sparsification Sensitivity (624 patients)',
            ...
            logger.info("Not enough valid results to generate plot")
